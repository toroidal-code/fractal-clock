...
static Size rotateSize(Rotator rotator, Size s0){
   Size newSize = { 
...
static double midX(Rect rect){
	return rect.size.width /2.;
}

static double midY(Rect rect){
	return rect.size.height /2.;
}

static Rect getRootAndRotators(Rect bounds, Rotator r0, Rotator r1) 
{
	double now = getNow();
	...
	root.size = rotateSize(r, rootSizeStruct);
                               ...
                               WPARAM wParam, LPARAM lParam)
{
	...
	static HGLRC hRC;
	static RECT rect;
	static Point origin = {1920/2., 1080/2.};
	static Size windowSize = {1920, 1080};
	static Rect windowRect = {origin, windowSize};
